...
@Slf4j
@Service
public class PtrInboxFilterService {
    ...
    public List<String> fetchApplicationNumbersFromSearcher(InboxSearchCriteria criteria, HashMap<String, String> StatusIdNameMap, RequestInfo requestInfo){
        ...
        if(!isSearchResultEmpty){
            ...
            if(moduleSearchCriteria.containsKey(PTR_APPLICATION_NUMBER_PARAM)) {
                searchCriteria.put(PTR_APPLICATION_NUMBER_PARAM, moduleSearchCriteria.get(PTR_APPLICATION_NUMBER_PARAM));
            }
            ...
            applicationNumbers = JsonPath.read(result, "$.PetRegistrationApplications.*.applicationnumber");
        }
